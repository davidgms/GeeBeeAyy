    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: GeeBeeDesign.spacingMD) {
                        // Header
                        HStack {
                            Text("Your Games")
                                .font(.system(size: GeeBeeDesign.fontTitle, weight: .bold))
                                .foregroundColor(.goldenSaplight)
                            Spacer()
                        }
                        .padding(.horizontal, GeeBeeDesign.spacingMD)
                    ...
                    .padding(.vertical, GeeBeeDesign.spacingMD)
                }
            }
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(action: {}) {
                        HStack {
                            Text("GeeBee-A!")
                                .font(.system(size: GeeBeeDesign.fontHeadline, weight: .bold))
                                .foregroundColor(.goldenSaplight)
                            Text("ROMs")
                                .font(.system(size: GeeBeeDesign.fontHeadline, weight: .light))
...
struct SectionHeader: View {
    ...
    var body: some View {
        Text(title)
            .font(.system(size: GeeBeeDesign.fontBody, weight: .semibold))
            .foregroundColor(.amberResin)
            .padding(.horizontal, GeeBeeDesign.spacingMD)
            .padding(.top, GeeBeeDesign.spacingSM)
    }
}
...
struct ROMCard: View {
    ...
    var body: some View {
        Button(action: {}) {
            HStack(spacing: GeeBeeDesign.spacingMD) {
                // Game icon placeholder
                ZStack {
                    RoundedRectangle(cornerRadius: GeeBeeDesign.radiusSM)
                ...
                VStack(alignment: .leading, spacing: GeeBeeDesign.spacingXS) {
                    Text(rom.name)
                        .font(.system(size: GeeBeeDesign.fontBody, weight: .semibold))
                        .foregroundColor(.pineGlowMist)
                        .lineLimit(1)
            ...
            .padding(GeeBeeDesign.spacingMD)
        }
        .buttonStyle(PlainButtonStyle())
        .background(Color.honeyDark)
        .cornerRadius(GeeBeeDesign.radiusMD)
        .padding(.horizontal, GeeBeeDesign.spacingMD)
    }
}
